...
export default function FlowingMenu({
  ...
  return (
    <div className="menu-wrap transition-colors duration-300" style={{ backgroundColor: bgColor }}>
      <nav className="menu">
        {items.map((item, idx) => (
          <MenuItem
            key={idx}
...
interface MenuItemComponentProps extends FlowingMenuItemType {
  speed: number;
  textColor: string;
  marqueeBgColor: string;
  marqueeTextColor: string;
...
function MenuItem({ link, text, image, speed, textColor, marqueeBgColor, marqueeTextColor, borderColor, onClick }: MenuItemComponentProps) {
  const itemRef = useRef<HTMLDivElement>(null);
  const marqueeRef = useRef<HTMLDivElement>(null);
  const marqueeInnerRef = useRef<HTMLDivElement>(null);
  const animationRef = useRef<gsap.core.Tween | null>(null);

  // Force initial state to prevent them from showing on mount
  useEffect(() => {
  ...
  return (
    <div className="menu__item" ref={itemRef} style={{ overflow: 'hidden', borderColor }}>
      <Link
        className="menu__item-link transition-colors duration-300"
        href={link}
